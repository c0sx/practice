...
pub fn winning_hands<'a>(hands: &[&'a str]) -> Vec<&'a str> {
    ...
    let max_hand: &str = hands
        .into_iter()
        .max_by(|a, b| compare_cards(a, b))
        .unwrap();

    result.push(max_hand);

    // find other hands, that equal to max

    let other = hands
        .into_iter()
        .filter(|one| is_equal(max_hand, one))
        .map(|one| *one)
        .collect::<Vec<&str>>();
...
fn map_card_value(value: &str) -> u32 {
...
fn get_max_card(hand: &str) -> &str {
    hand
        .split_whitespace()
        .max_by(|a, b| {
            let max_a = map_card_value(a);
            let max_b = map_card_value(b);
...
fn compare_cards(a: &str, b: &str) -> Ordering {
    let max_a = get_max_card(a);
    let max_b = get_max_card(b);

    let max_a = map_card_value(max_a);
    let max_b = map_card_value(max_b);

    return max_a.cmp(&max_b);
}

fn is_equal(a: &str, b: &str) -> bool {
    compare_cards(a, b) == Ordering::Equal
}
